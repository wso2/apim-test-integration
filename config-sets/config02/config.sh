...
appPath=repository/deployment/server/webapps/

prgdir=$(dirname "$0")
configDir=$(cd "$prgdir"; pwd)

#copy config files
\cp -f $configDir/files/output-event-adapters.xml $productHome/$filePath
\cp -f $configDir/files/resource.war $productHome/$appPath
cd $productHome/bin
