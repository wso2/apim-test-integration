...
sudo chmod 755 $APIM_HOME/bin/api-manager.sh
nohup sh "$APIM_HOME/bin/api-manager.sh" -Dmigrate -DmigrateFromVersion="$SOURCE_APIM_VERSION" -DmigratedVersion="$APIM_VERSION" > "$APIM_MIGRATION_LOG_FILE" 2>&1 &
SERVER_PID=$!
...
tail -n0 -F "$APIM_MIGRATION_LOG_FILE" | while true; do
    ...
    if grep -qE 'ERROR' <<< "$line"; then
        echo "Fatal error detected during APIM migration:"
        echo "$line"
        echo "------ IS Log file content ------"
        cat "$IS_MIGRATION_LOG_FILE"
        echo "------ APIM Log file content ------"
