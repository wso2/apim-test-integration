            ...
            try {
                Utils.getApiToken()
                    .then((token) => {
                        const curl = `curl -k -X POST \
                        -H "Content-Type: application/json" \
                        -H "Authorization: Bearer ${token}"  "${Cypress.config().baseUrl}/api/am/publisher/v2/apis/change-lifecycle?action=Publish&apiId=${apiId}"`;
                        cy.exec(curl).then(result => {
            ...
            try {
                Utils.getApiToken()
                    .then((token) => {
                        ...
                        -H "Content-Type: application/json" \
                        -H "Authorization: Bearer ${token}"  "${Cypress.config().baseUrl}/api/am/publisher/v2/apis/${apiId}"`;
                        cy.exec(curl).then(result => {
                            resolve(result.stdout);
                        })
                    })
            ...
            try {
                Utils.getApiToken()
                    .then((token) => {
                        const curl = `curl -k -X DELETE \
                        -H "Content-Type: application/json" \
                        -H "Authorization: Bearer ${token}"  "${Cypress.config().baseUrl}/api/am/publisher/v2/api-products/${productId}"`;
                        cy.exec(curl).then(result => {
                            resolve(result.stdout);
                        })
                    })
