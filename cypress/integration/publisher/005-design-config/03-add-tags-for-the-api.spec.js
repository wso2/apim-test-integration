...
describe("Add tags for the api", () => {
  ...
  it("Add tags for the api", () => {
    Utils.addAPI({ name: apiName, version: apiVersion }).then((apiId) => {
      ...
      cy.get('label').contains('Tags').invoke('css', 'pointer-events', 'auto');
      cy.contains('label', 'Tags').scrollIntoView().wait(3000).click({ force: true });
      cy.wait(3000);
      cy.contains('label', 'Tags').type(`${tagName}{enter}`);
      cy.get('label').contains('Tags').invoke('css', 'pointer-events', 'none');
